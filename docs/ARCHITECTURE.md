...
choice and software path exists to keep the repo easily observable. This file is
the top-level overview of how the system is put together.
...
| ---------- | -------- | -------------------------- | ------------------------------------------ |
| Controller | head     | Ryzen 7 PRO 4750U  / 16 GB | Runs PXE, Apache, dnsmasq                  |
...
mini-pcs I bought on sale. Networking stays simple, Cat6e cables into a
2.5 GbE switch and a flat `/24` LAN.
...
4. **Ansible Roles**: `make controller-venv` (one time) prepares the controller
  virtualenv, then `make controller` runs `controller_common → controller → pxe`.
  `make compute` currently runs only `compute_common`, which seeds SSH access and
  verifies the preinstalled Python runtime so future workloads can be layered in.
  Re-running the targets is safe.

### Role Stack Per Host

- **Controller**: `controller_common → controller → pxe`
- **Compute**: `compute_common` (SSH bootstrap + Python validation; future scheduler TBD)
